...
class Tree:
    ...
    def add(self, point1, point2, velocity, control=None, duration=None):

        # print(point1, point2,'checker')
        if point1 == point2:
            # print('heree')
    ...
    def extend(self, point, n1, n2, dt):
        ...
        for _ in range(10):
            ...
            nearest_node_vel = [*nearest_node, *self.velocity[nearest_node]]
            q_new, q_new_dot = self.robot.propagate(nearest_node_vel, controls, duration, dt)
            # q_new = tuple(q_new)
            q_new_pos, q_new_vel = tuple(q_new[:3]), q_new[3:]
            # print('new position and velocity', nearest_node_vel, q_new_pos, q_new_vel)
            # q_new_dot = q_new_dot
            collider = False
            # print('collision', isCollisionFree(self.robot, q_new_pos, self.obstacles))
            if isCollisionFree(self.robot, q_new_pos, self.obstacles):
                # q_near = nearest_node
                # discretized = 20
                ...
                if not collider:
                    # print('adding', q_new)
                    nearest_nodes[q_new_pos] = (q_new_vel, c[0], d[0])
                    j += 1
                # else:
                #     q_new = q_near
